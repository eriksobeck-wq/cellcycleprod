...
# DISCHARGE_CURRENT       = 2.0  # Deprecated - now using constant power mode below
DISCHARGE_POWER         = 5.0   # Watts - constant power discharge (adjust as needed for your test)
...
# === Fan Control Functions === (unchanged)
def set_fan(state):
...
print("=== Remote Sense Cell Cycler with Live Plot + Fan Control (Constant Power Discharge) ===\n")

# === Instrument Setup === (unchanged)
...
def run_battery_cycle(max_cycles=1):
    ...
    with open(LOG_FILE, 'w', newline='') as f:
        ...
        for cycle in range(1, max_cycles + 1):
            ...
            # === CHARGE REST (force fan ON) === (unchanged)
            ...
            # === DISCHARGE PHASE === (MODIFIED for constant power)
            print("\n>>> DISCHARGE PHASE STARTED")
            print(f"    Using constant power discharge: {DISCHARGE_POWER} W")
            # Set constant power (POW / CW) mode - SCPI commands per 8500B series manual
            load.write('SOUR:MODE POW')
            load.write(f'SOUR:POW {DISCHARGE_POWER}')
            load.write('INP ON')
            ...
            # === DISCHARGE REST (force fan ON) === (unchanged)
